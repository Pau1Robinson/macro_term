...
def get_valid_keypress_input(input_question, invalid_input_response):
    '''
    asks the user for input using the input-question argument and returns that input if it is valid or informs the user their input is\n
    invalid using invalid_input_response\n
    input_question: string outputted to the user when the input is requested\n
    invalid_input_response: the string sent to the user when their input is invalid to inform them their input is invalid\n
    '''
    user_input = input(input_question)
    valid_keyboard_press = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '-', '=', 'q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p', '[', ']', 'a', 's', 'd', 'f',
...
def get_valid_keypress_string(string_input_question, invalid_string_response):
    '''
    Asks the user for a string of keypresses using string_input_question and check if those keypresses are valid/n
    string_input_question: String outputted to the user when the input is requested/n
    invalid_string_response: The string sent to the user when the string they inputted is invalid to inform them their string is invalid\n
    '''
...
def get_valid_filename(filename_input_question, invalid_filename_response):
    '''
    Asks the user to input a filename using filename_input_question and check if they inputted a valid filename/n
    either returning the filename if its valid or printing invalid_filename_response if its not/n
...
def get_valid_file_path(path_input_question, invalid_path_response):
    '''
    ...
    path_input_question: The string to be outputted to the user when asking for the file path\n
    invalid_path_response: The string to be outputted to the user if the file path is invalid
    '''
    path_input = input(path_input_question)#NTS add support for filename without path?
...
def create_macro(keypress_input, keypress_string, create_dict):
    '''
    Takes input from keypress_input and keypress_string and writes it into create_dict to store the macro/n
...
def listening_mode(listen_dict):
    ...
    def on_press(key):
        '''
        uses currently_pressed to keep track of the keys being presses, then check if those keys are a macro shortcut/n
    ...
    def on_release(key):
        '''
        uses currently_pressed_macro to check if a macro shortcut is pressed and on the release of that shortcut activates the macro/n
        using output_macro
        '''
        try:
    ...
    def output_macro(activated_macro, output_dict):
        '''
        checks output_dict for the key activated_macro and then outputs the value of activated_macro using pynput keyboard controller
        '''
...
def main():
    ...
    macro_dict = {}
    options_menu()
    create_string = 'your macro will be activated by holding alt and a key of your choice. please type the key you would like to use\n'
    main_input = input()
    while main_input != '7':
        if main_input == '1':
            macro_dict = create_macro(get_valid_keypress_input(create_string, 'that is not a valid key'),
                                      get_valid_keypress_string('enter the keystrokes you would like your macro to output\n',
                                      'those are not valid keystokes'), macro_dict)
            print(macro_dict)
            main_input = input()
        elif main_input == '4':
            macro_dict = import_macro(get_valid_file_path('Enter the path of the file you want to input\n', 'That is not a valid file path'), macro_dict)
            main_input = input()
        elif main_input == '5':
